                ...
                if onScreen then
                    ...
                    if ESP_SETTINGS.ShowBox and ESP_SETTINGS.Enabled then
                        ...
                        elseif ESP_SETTINGS.BoxType == "Corner Box Esp" then
                            ...
                            -- inline
                            local function UpdateBoxLines()
                                for i = 9, 16 do
                                    if boxLines[i] then -- Ensure the line exists before updating
                            ...
                            -- Callback for updating the color
                      -- Store the last color applied to avoid unnecessary updates
                        local lastBoxColor = ESP_SETTINGS.BoxOutlineColor

                        -- Function to update the box lines with the current color
                        local function UpdateBoxLines()
                            for i = 9, 16 do
                                if boxLines[i] then  -- Ensure the box line exists
                                    boxLines[i].Thickness = 2
                                    boxLines[i].Color = ESP_SETTINGS.BoxOutlineColor
                                    boxLines[i].Transparency = 1
                                end
                        ...
                        -- Function to check if the color has changed, and update if necessary
                        local function CheckAndUpdateColor()
                            if ESP_SETTINGS.BoxOutlineColor ~= lastBoxColor then
                                lastBoxColor = ESP_SETTINGS.BoxOutlineColor  -- Update the last color
                                UpdateBoxLines()  -- Update the box lines with the new color
                            end
                        end

                        -- Call this function periodically to check for updates (without lag)
                        while true do
                            CheckAndUpdateColor()
                            wait(0.1)  -- Wait for a short time before checking again to avoid lag (adjust if necessary)
                        end
