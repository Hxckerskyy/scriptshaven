...
local Library = {
    ...
    AccentColor = Color3.fromRGB(148, 29, 184);
    OutlineColor = Color3.fromRGB(50, 50, 50);
    RiskColor = Color3.fromRGB(255, 50, 50);

    Black = Color3.new(0, 0, 0);

    -- Font setup with fallback to default font in case custom font isn't available
    Font = Enum.Font.SourceSans,  -- Default fallback font
...
if isfile("ProggyClean.ttf") then
    Library.Font = Font.new("rbxassetid://" .. getcustomasset("ProggyClean.ttf"))
end
...
if Library.IsMobile then
	Library.MinSize = Vector2.new(500, 350);
end
...
function Library:CreateLabel(Properties, IsHud)
    -- Ensure the custom font is set
    local font = Enum.Font.SourceSans -- Default fallback font
    
    -- Check if the custom font is available (you can adjust this as needed)
    if isfile("ProggyClean.ttf") then
        font = Font.new("rbxassetid://" .. getcustomasset("ProggyClean.ttf"))
    end

    -- Create the TextLabel instance with the font
    local _Instance = Library:Create('TextLabel', {
        BackgroundTransparency = 1;
        Font = font;  -- Use the custom font here
    ...
    -- Add to registry for further customization
    Library:AddToRegistry(_Instance, {
        TextColor3 = 'FontColor';
    }, IsHud);

    -- Return the label with the provided properties
...
function Library:CreateWindow(...)
	...
	if type(Config.ShowCustomCursor) ~= 'boolean' then Library.ShowCustomCursor = false else Library.ShowCustomCursor = Config.ShowCustomCursor end
